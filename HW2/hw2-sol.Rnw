%Note: set our Rstudio settings to compile as Sweave.
...
Let $V = \Var[h(X)]$, $\hat{V} = \frac{1}{n^2}\sum_{i=1}^n[h(X_i)-\hat{I}]^2$ would be the estimator.

\item[(3)] $\hat{I}$ follows $\mathcal{N}(I, V)$ when sample size is large, according to the Central Limit Theorem (CLT). Hence, the 95\% confidence interval is given as $[\hat{I}-z_{.025}\sqrt{\hat{V}}, \hat{I}+z_{.025}\sqrt{\hat{V}}]$, where $z_{.025}=1.96$.
