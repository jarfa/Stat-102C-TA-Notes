...
&= \sum_z K(x,z) K(z,y) = K*K \\
K^{(2)}(x,y) &= K^2(x,y)
\end{align}

\section*{Problem 1.4}
In 1.1, suppose a population of 1 million people is moving around our list of cities. $p^{(t+1)}(y)$ can be interpreted as the number of people in city $y$ at time $t+1$, and $p^{(t)}(x)$ can be interpreted as the number of people in city $x$ at time $t$. $K(x, y)$ is the fraction
can be interpreted as, among all the people in city $x$ at time $t$, the fraction of who will move to $y$ at time $t+1$. So $p^{(t)}(x) K(x, y)$ is the number of people who are at $x$ at time $t$ and who are in $y$ at time $t+1$. So $p^{(t+1)}$ should be the sum of $p^{(t)}(x) K(x, y)$ over all cities $x$.  \\ \\
\noindent
...
\\ \\
\noindent
In 1.3, The transition matrix for time $s$, $K^{(s)}(x, y)$ can be  intrepreted as, among all the people in city $x$ at time $t$, the fraction of those who will end up in city $y$ at time $t+s$. 
...
Intrpretation of $\pi(x) K(x, y)= \pi(y) K(y, x)$. This is detailed balance, meaning that for every pair of different states $x, y$, the number of people who go from $x$ to $y$ is the same as the number of people who go from $y$ to $x$. If this is true for every pair of states, then the number of people in each state does not change. 
