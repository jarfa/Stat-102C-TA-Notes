...
#so regular MC integration will converge to the true value
...
f = function(x) dbeta(x, 5, 5)
weights = f(x)/g(x)
mean((weights * h(x)) < 0.2) #no, that's not right...
indicators = (h(x) < 0.2) #the indicators of h < 0.2 is analagous to h(x)
mean(weights * indicators) #yes!
