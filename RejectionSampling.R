...
samplefrom = function(gsamples){
  ...
  u = runif(1) * M
  #     u > f(y)     /   g(y)
  while(u > dnorm(y) / dunif(y, min=-4, max=4)){
    u = runif(1) * M
    y = sample(gsamples, 1) 
  }
  return(y)
...
a = 2.7
b = 6.3
optimize(f = function(x) dbeta(x, a, b), interval=c(0,1), maximum=TRUE)$objective
...
#~37% of samples we generated from the uniform were inside the beta.

par(mfrow = c(1,1))
plot(density(rbeta(1e5, 2.7, 6.3)), col='red', main = 'Beta Desnsities', ylim = c(0,3))
lines(density(rbeta(1e5, 2, 6)), col='blue')
legend(x=0.5,y=3, c('a, b = 2.7, 6.3', 'a, b = 2, 6'), lty=1, col = c('red', 'blue'))
